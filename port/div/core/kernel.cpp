...
/* PORT: la direccion va a un temporal. El original hacia
 * `pila[sp]=mem[pila[sp]]++;`, donde pila[sp] es el operando Y el destino:
 * sin punto de secuencia entre los dos efectos, MSVC asignaba antes de
 * incrementar y el incremento caia en mem[valor_antiguo]. Sintoma: `x++`
 * no hacia nada (`FOR (n=0;n<k;n++)` se colgaba creando procesos). */
case lipt: { int dir=pila[sp]; pila[sp]=++mem[dir]; } break;
case lpti: { int dir=pila[sp]; pila[sp]=mem[dir]++; } break;
case ldpt: { int dir=pila[sp]; pila[sp]=--mem[dir]; } break;
case lptd: { int dir=pila[sp]; pila[sp]=mem[dir]--; } break;
...
/* PORT: mismo arreglo de orden de evaluacion que en lipt/lpti (ver arriba). */
case liptchr: { int dir=pila[sp-1]*4+pila[sp]; pila[sp-1]=(unsigned int)++memb[dir]; sp--; } break;
case lptichr: { int dir=pila[sp-1]*4+pila[sp]; pila[sp-1]=(unsigned int)memb[dir]++; sp--; } break;
case ldptchr: { int dir=pila[sp-1]*4+pila[sp]; pila[sp-1]=(unsigned int)--memb[dir]; sp--; } break;
case lptdchr: { int dir=pila[sp-1]*4+pila[sp]; pila[sp-1]=(unsigned int)memb[dir]--; sp--; } break;
case ladachr:
  pila[sp-2]=(unsigned int)(memb[pila[sp-2]*4+pila[sp-1]]+=(byte)pila[sp]); sp-=2; break;
case lsuachr:
  pila[sp-2]=(unsigned int)(memb[pila[sp-2]*4+pila[sp-1]]-=(byte)pila[sp]); sp-=2; break;
...
/* PORT: mismo arreglo de orden de evaluacion que en lipt/lpti (ver arriba). */
case liptwor: { int dir=pila[sp-1]*2+pila[sp]; pila[sp-1]=(unsigned int)++memw[dir]; sp--; } break;
case lptiwor: { int dir=pila[sp-1]*2+pila[sp]; pila[sp-1]=(unsigned int)memw[dir]++; sp--; } break;
case ldptwor: { int dir=pila[sp-1]*2+pila[sp]; pila[sp-1]=(unsigned int)--memw[dir]; sp--; } break;
case lptdwor: { int dir=pila[sp-1]*2+pila[sp]; pila[sp-1]=(unsigned int)memw[dir]--; sp--; } break;
case ladawor:
  pila[sp-2]=(unsigned int)(memw[pila[sp-2]*2+pila[sp-1]]+=(word)pila[sp]); sp-=2; break;
case lsuawor:
  pila[sp-2]=(unsigned int)(memw[pila[sp-2]*2+pila[sp-1]]-=(word)pila[sp]); sp-=2; break;
