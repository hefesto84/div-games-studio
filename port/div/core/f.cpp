...
int get_reloj(void) {
  /* PORT: devuelve un valor en vivo (no el campo 'reloj' cacheado) para
   * que el bucle de espera de frame_start() (i.cpp) pueda detectar el
   * paso del tiempo real mientras espera -- ver el comentario junto a
   * port_get_reloj() en port/div/shim/dos.h/port_misc.cpp y
   * docs/architecture/12-port-progreso.md checkpoint 8/9. */
  return port_get_reloj();
}
...
/* PORT (E5): el original elegia la cadena de busqueda permisiva solo en el
 * build DEBUG (SESSION.386, el que el IDE de DOS lanzaba al "Probar"). Es la
 * unica que respeta subcarpetas como "tutorial\\tutor0.fpg", asi que el port la
 * usa siempre. Ver docs/architecture/13-handoff.md SS5.5. */
#if defined(DEBUG) || defined(PORT_OPEN_FILE_SEARCH)
    ...
    if ((f=fopen(full,"rb"))==NULL) {                   // "est\paz\fixero.est"
      ...
      if ((f=fopen(full,"rb"))==NULL) {                 // "fixero.est"
        ...
        if ((f=fopen(full,"rb"))==NULL) {               // "est\fixero.est"
          strcpy(full,"");
          return(port_open_file_extra(file)); /* PORT (E5): DIV_RES_PATH */
        } else return(f);
      } else return(f);
    } else return(f);
  } else return(f);
...
void _fopen(void) { // Busca el archivo, ya que puede haber sido incluido en la instalaci�n
  ...
  /* PORT: el original hacia "if (pila[sp]=(int)f)" -- probaba si f no es
   * NULL Y de paso truncaba el FILE* a 32 bits dentro de pila[sp] (que es
   * int, el tipo de palabra de la VM: eso no se toca). El problema era
   * que tabfiles[x]=pila[sp] copiaba ese valor YA truncado. Se separa el
   * test de f (sin truncar) del valor que efectivamente se guarda en
   * tabfiles (ahora intptr_t, ver inter.h); pila[sp] mantiene el mismo
   * comportamiento final en todos los caminos (0 si fallo, x*2+1 si
   * exito) que tenia el original. Ver docs/architecture/12-port-progreso.md. */
  pila[sp]=0;
  if (f) {
...
void _fclose(void) {
  ...
  if (pila[sp]==0) {
    ...
    /* PORT: era "32*4"; tabfiles ahora es intptr_t (ver inter.h),
     * sizeof(tabfiles) es correcto en cualquier ancho de puntero. */
    memset(tabfiles, 0, sizeof(tabfiles));
  } else {
...
void _malloc(void) {
  ...
  /* PORT: la alineacion a multiplo de 4 usa uintptr_t en vez de (int) por
   * el mismo motivo que "ghost"/mem[] mas arriba en este mismo arbol
   * (evita truncar el puntero real en x64). El calculo de imem1 (linea
   * de abajo) tambien se hace ahora en uintptr_t antes de truncar a int,
   * lo cual evita el caso claramente peor de truncar "p" y "mem" por
   * SEPARADO antes de restar (eso si daba una diferencia arbitraria/
   * basura en 64 bits). RIESGO ARQUITECTONICO NO RESUELTO, documentado
   * en vez de ocultado (ver docs/architecture/12-port-progreso.md): este
   * "imem1" pretende ser un indice dentro de mem[] que el bytecode DIV
   * pueda usar como si "p" viviera dentro del propio array mem[], pero
   * "p" en realidad viene de un malloc() independiente. En el DOS
   * original ambos vivian en el mismo espacio plano de 32 bits, asi que
   * la resta siempre caia dentro de un int valido; en un proceso x64 NO
   * hay ninguna garantia de que malloc() devuelva algo a una distancia
   * de mem[] que quepa en un int tras dividir por 4. Si eso ocurre, el
   * resultado sigue siendo incorrecto (aunque ya no por truncar de forma
   * prematura). Arreglo de fondo pendiente: reservar la memoria de
   * divmalloc desde dentro del propio mem[] en vez de con malloc()
   * aparte. */
  p=(byte*) ( ( ( (uintptr_t) divmalloc[con].ptr+3) /4)*4 );

  divmalloc[con].imem1=(int)(((uintptr_t)p-(uintptr_t)mem)/4);
