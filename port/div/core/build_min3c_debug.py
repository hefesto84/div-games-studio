# DEBUG checkpoint 14: variante de build_min3a/b. Hijo con movimiento
# (get_distx/get_disty) Y rotacion (angle+=incr) simultaneos, empezando
# cerca de la esquina inferior derecha (100,460) para que salga de pantalla:
# ejercita sp_rotado + recortes (clip) en los 4 bordes, caso que el anillo
# centrado de api_demo no cubre. Si no crashea en 10 s, la ruta de rotacion
# con clipping es robusta.
import struct, zlib
...
with open('test_min3c.div32', 'wb') as f:
    f.write(b'\x00' * 602)
    f.write(header9)
    f.write(struct.pack('<I', n))
    f.write(comprimido)
print(f"OK min3c: entry={code_base} hijo={hijo_addr} mem8={mem8} n={n}")
