...
void pinta_sprites_m7(int n,int cx,int cy,float ang) { // Le pasamos la posici�n de la c�mara
  ...
  do {
    ...
    if (ide) {
      if (max>=32) { // No pinta el objeto si est� en las coordenadas de la c�mara (�2)
...
//����������������������������������������������������������������������������
// Collision(tipo) - deteccion de colisiones entre procesos
//����������������������������������������������������������������������������

/* PORT: el original (src/div/scr_man.cpp, en parte ensamblador) hacia una
 * deteccion por CAJA del grafico de cada proceso escalado por _Size,
 * ignorando el angulo de rotacion (comportamiento documentado de collision()
 * en DIV: el angulo del proceso no se tiene en cuenta para la colision).
 * Sustituye al stub vacio de port_stubs.c. La caja se calcula con la misma
 * math que sp_escalado(): pivote del grafico (anmac), espejados _Flags y
 * redondeo (w*size)/100, de modo que la colision coincide con lo que se
 * dibuja en pantalla. Para procesos en scroll (_Ctype==1) se aplica el mismo
 * desplazamiento que pinta_sprite(). */

static int col_caja(int p, int *x0, int *y0, int *x1, int *y1)
{
  int file, graph, an, al, xg, yg, x, y, size, flags;
  int * ptr;

  file=mem[p+_File];
  if (file<0 || file>max_fpgs) return 0;
  if (g[file].grf==NULL) return 0;

  graph=mem[p+_Graph];
  if (graph<=0) return 0;
  if (file==0) { if (graph>=2000) return 0; }
  else         { if (graph>=1000) return 0; }
  ptr=g[file].grf[graph];
  if (ptr==NULL) return 0;

  an=ptr[13]; al=ptr[14];
  if (an<=0 || al<=0) return 0;

  size=mem[p+_Size];
  if (size<=0) return 0;

  if (ptr[15]==0 || *((word*)ptr+32)==65535) { xg=an/2; yg=al/2; }
  else { xg=*((word*)ptr+32); yg=*((word*)ptr+33); }
  ...
  x=mem[p+_X]; y=mem[p+_Y];
  if (mem[p+_Resolution]>0) { x/=mem[p+_Resolution]; y/=mem[p+_Resolution]; }
  if (mem[p+_Ctype]==1) {
    x+=iscroll[snum].x-iscroll[snum].map1_x;
    y+=iscroll[snum].y-iscroll[snum].map1_y;
  }

  *x0=x-(xg*size)/100; *y0=y-(yg*size)/100;
  *x1=*x0+(an*size)/100-1;
  *y1=*y0+(al*size)/100-1;
...
void collision(void)
{
  int tipo, i, p0, q0, p1, q1, j0, k0, j1, k1;

  tipo=pila[sp];
  /* PORT fix: el id de tipo es (int)(struct objeto*) generado por el
   * compilador; en x64 puede ser negativo. La original (c.cpp:157) solo
   * trata bloque==0 (collision con el raton) como caso especial, los
   * negativos son ids validos. Rechazar tipo<0 rompia TODAS las colisiones
   * de programas compilados de verdad (p.ej. los disparos de STEROID). */
  if (tipo==0) { pila[sp]=0; return; }

  if (!col_caja(id,&p0,&q0,&p1,&q1)) { pila[sp]=0; return; }

  for (i=id_init; i<=id_end; i+=iloc_len) {
    if (i==id) continue;
    if (mem[i+_Bloque]!=tipo) continue;
    if (mem[i+_Status]!=2 && mem[i+_Status]!=4) continue;
    if (!col_caja(i,&j0,&k0,&j1,&k1)) continue;
    if (p0<=j1 && j0<=p1 && q0<=k1 && k0<=q1) { pila[sp]=i; return; }
  }
...
//
// Out_region(id,region) - 1 si el grafico del proceso esta COMPLETAMENTE
// fuera de la region, 0 si la toca. Sustituye al stub vacio de port_stubs.c
// (que dejaba la pila intacta: devolvia el argumento 'region', o sea 0 para
// out_region(id,0) -> el REPEAT..UNTIL de muerte_jack() de MALVADO no
// terminaba nunca: ni vidas-- ni respawn).
//
// Portado de c.cpp:30-109 del runtime original, con sp_size/sp_size_scaled
// copiadas como estaticas (c.cpp no entra en el build del port).
//

static void port_sp_size(int *x,int *y,int *xx,int *yy,int xg,int yg,
                         int ang,int size,int flags)
{
  float d0,d1,d2,d3;
  float a0,a1,a2,a3;
  ...
  if (flags&1) {
    pp[0]=*x-(int)((float)cos(a0)*d0); pp[2]=*x-(int)((float)cos(a1)*d1);
    pp[4]=*x-(int)((float)cos(a2)*d2); pp[6]=*x-(int)((float)cos(a3)*d3);
  } else {
    pp[0]=*x+(int)((float)cos(a0)*d0); pp[2]=*x+(int)((float)cos(a1)*d1);
    pp[4]=*x+(int)((float)cos(a2)*d2); pp[6]=*x+(int)((float)cos(a3)*d3);
  }
  ...
  if (flags&2) {
    pp[1]=*y-(int)(-(float)sin(a0)*d0); pp[3]=*y-(int)(-(float)sin(a1)*d1);
    pp[5]=*y-(int)(-(float)sin(a2)*d2); pp[7]=*y-(int)(-(float)sin(a3)*d3);
  } else {
    pp[1]=*y+(int)(-(float)sin(a0)*d0); pp[3]=*y+(int)(-(float)sin(a1)*d1);
    pp[5]=*y+(int)(-(float)sin(a2)*d2); pp[7]=*y+(int)(-(float)sin(a3)*d3);
  }

  *x=0x7fffffff; *y=0x7fffffff; *xx=(int)0x80000000; *yy=(int)0x80000000;

  for (n=0;n<8;n++) {
    if (*x>pp[n]) *x=pp[n]; if (*xx<pp[n]) *xx=pp[n]; n++;
    if (*y>pp[n]) *y=pp[n]; if (*yy<pp[n]) *yy=pp[n];
  }
}

static void port_sp_size_scaled(int *x,int *y,int *xx,int *yy,int xg,int yg,
                                int size,int flags)
{
  int x0,y0,x1,y1;
...
{
  int pid,file,graph,reg,angle;
  int xg,yg,xg1,yg1,x,y,an,al;
  int * ptr,n,m;

  reg=pila[sp--]; pid=pila[sp]; pila[sp]=1; // Por defecto esta fuera de la region

  if (mem[pid+_Ctype]==2) { e(137); return; }

  file=mem[pid+_File]; graph=mem[pid+_Graph]; angle=mem[pid+_Angle];

  if (reg<0 || reg>=max_region) { e(108); return; }
  if (file<0 || file>max_fpgs) { e(109); return; }

  if ((n=mem[pid+_XGraph])>0) {
    m=mem[n]; if (m<1 || m>256) return;
    ...
    if (graph>=m) graph=0;
    if ((graph=mem[n+1+graph])<0) { graph=-graph; mem[pid+_Flags]|=1; }
  }
  ...
  x=mem[pid+_X]; y=mem[pid+_Y];
  if (mem[pid+_Resolution]>0) { x/=mem[pid+_Resolution]; y/=mem[pid+_Resolution]; }
  ...
  if (angle) {
    port_sp_size(&x,&y,&an,&al,xg,yg,angle,mem[pid+_Size],mem[pid+_Flags]);
    an=an-x+1; al=al-y+1;
  } else if (mem[pid+_Size]!=100) {
    port_sp_size_scaled(&x,&y,&an,&al,xg,yg,mem[pid+_Size],mem[pid+_Flags]);
    an=an-x+1; al=al-y+1;
  } else {
    if (mem[pid+_Flags]&1) xg=an-1-xg; x-=xg;
    if (mem[pid+_Flags]&2) yg=al-1-yg; y-=yg;
  }

  // Ya hay region[reg].(x0..y1) y sprite(x,y,an,al)

  if (mem[pid+_Ctype]==1) {

    for (n=0;n<10;n++) if (iscroll[n].on && (mem[pid+_Cnumber]==0 || (mem[pid+_Cnumber]&(1<<n)))) {

      xg=x+iscroll[n].x-iscroll[n].map1_x; // Posicion del sprite en el scroll
      yg=y+iscroll[n].y-iscroll[n].map1_y;

      xg1=xg+an; yg1=yg+al; // Interseccion de xg,yg,an,al con la ventana del scroll
      ...
      if (xg>=xg1 || yg>=yg1) continue; // Si no hay interseccion, continua

      if (xg<region[reg].x1 && yg<region[reg].y1 && xg1>region[reg].x0 &&
          yg1>region[reg].y0) { pila[sp]=0; return; } // Si esta en la region

    }

  } else if (mem[pid+_Ctype]==0) {

    if (x<region[reg].x1 && y<region[reg].y1 && x+an>region[reg].x0 &&
        y+al>region[reg].y0) pila[sp]=0; // Si esta en la region
