...
*/
  ...
  /* PORT: (int) truncaba el puntero a 32 bits en x64 (bug real, no solo un
   * aviso del compilador) antes de aplicar la mascara de alineacion a 256
   * bytes. Se cambia a uintptr_t Y la mascara a ~(uintptr_t)0xFF (limpia
   * solo los 8 bits bajos, igual que 0xFFFFFF00 hacia en 32 bits) para no
   * perder los bits altos de la direccion en un proceso de 64 bits.
   * Ver docs/architecture/12-port-progreso.md. */
  ghost=(char *)((uintptr_t)(ghost_inicial+512)&~(uintptr_t)0xFF);
...
// variables publicas
  ...
  /* PORT: era "32*4" (asumia sizeof(int)==4 por entrada); tabfiles ahora
   * es intptr_t (ver inter.h). sizeof(tabfiles) es correcto en cualquier
   * ancho de puntero. */
  memset(tabfiles, 0, sizeof(tabfiles));

/////////////////////////////////////////////////////////////

  /* PORT: CNT_export/CMP_export toman "char*" y COM_export_t espera
   * "const char*" -- MSVC en C++ no convierte implicitamente punteros a
   * funcion que solo difieren en la constancia de un parametro (Watcom si
   * lo permitia). Cast explicito, sin cambiar ninguna de las dos firmas
   * reales. Ver docs/architecture/12-port-progreso.md. */
  COM_export=(COM_export_t)CNT_export;
  LookForAutoLoadDlls();
  COM_export=(COM_export_t)CMP_export;
...
/* PORT: las tres funciones de abajo guardaban el FILE*-like "int* p" con
 * "mem[id+_SP]=(int)p" -- truncando un puntero real de malloc() dentro de
 * una celda de mem[], que por diseno de la VM tiene que seguir siendo
 * "int" de 32 bits (es el propio formato del bytecode/proceso, no algo
 * que se pueda ensanchar). En x64 eso truncaba el puntero de verdad. Se
 * sustituye por un handle pequeno (port_native_handle_*, ver
 * port_native_handles.h) que si cabe en un int; mem[id+_SP] sigue
 * significando "0 = no hay pila guardada" exactamente igual que antes.
 * Ver docs/architecture/12-port-progreso.md. */
void guarda_pila(int id, int sp1, int sp2) {
  int n, * p, h;
  p=(int*)malloc((sp2-sp1+3)*sizeof(int));
  if (p!=NULL) {
    h=port_native_handle_alloc(p);
    if (h==0) { free(p); mem[id+_SP]=0; return; } /* tabla de handles llena: mismo camino que "sin memoria" */
    mem[id+_SP]=h; p[0]=sp1; p[1]=sp2;
...
void carga_pila(int id) {
  int n, * p;
  if (mem[id+_SP]) {
    p=(int*)port_native_handle_get(mem[id+_SP]);
    for (n=0;n<=p[1]-p[0];n++) pila[p[0]+n]=p[n+2];
    port_native_handle_free(mem[id+_SP]);
    mem[id+_SP]=0; sp=p[1];
    free(p);
  } else sp=0;
}
...
void frame_start(void) {
  ...
  /* PORT: sincroniza el snapshot de 'reloj' con el reloj real en vivo
   * (ver get_reloj() en f.cpp) antes de que el resto de esta funcion lo
   * lea directamente (control de screen saver, contabilidad de fps,
   * decision de saltar el volcado). Ver docs/architecture/12-port-progreso.md
   * checkpoint 8/9. */
  reloj=get_reloj();
  ...
  if (reloj>(freloj+ireloj/3)) { // Permite comerse hasta un tercio del sgte frame
    ...
    do {
      n++;
      port_frame_yield(); /* PORT: ver dos.h -- evita que este spin sea tan
                              rapido que 'reloj' nunca llegue a avanzar entre
                              dos vueltas (ver checkpoint 9 del documento de
                              progreso). */
...
void frame_end(void) {
  ...
  /* PORT: la musica la mezcla libmikmod (io_song.c), cuyo driver winmm es
   * poll-driven -- sin una MikMod_Update() por frame la cancion se congela
   * a los ~2 buffers iniciales. Ver port/div/shim/dos.h. */
  io_song_update();
...
//����������������������������������������������������������������������������
//      Mensajes de error no criticos - Versi�n sin debugger
//����������������������������������������������������������������������������
...
void main(int argc,char * argv[]) {
  ...
  if ((mem=(int*)malloc(4*imem_max+1032*5+16*1025+3))!=NULL){
    ...
    /* PORT: mismo bug de alineacion que "ghost" (ver mas abajo en este
     * fichero) -- (int) truncaba el puntero a 32 bits en x64 antes de
     * alinear a un multiplo de 4. uintptr_t conserva la misma operacion
     * sin perder los bits altos de la direccion. */
