...
/* PORT (hito C1): includes DOS (i86/bios/dos/graph), JUDAS (audio) y
 * divmap3d.hpp eliminados -- el compilador no los usa; las cabeceras
 * estandar se adelantan via /FI port_compiler_pre.h. math.h tampoco:
 * divc.cpp define su propia exp2() (colision con el exp2() de C99). */
#include <conio.h>
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
...
typedef struct _pcminfo{
        ...
        int    SoundSize;
        short  *SoundData;
        void *sample; /* PORT (hito C1): era SAMPLE* (tipo de judas.h, no usado por divc.cpp) */
} pcminfo;
...
typedef struct {
  ...
  byte  fpg_path[256];
  byte  fpg_name[16];
  int   map; /* PORT (hito C1): era "tmap map" (tipo de divmap3d.hpp, no usado por divc.cpp) */
} M3D_info;
