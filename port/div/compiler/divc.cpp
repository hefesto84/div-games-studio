...
//    fclose(f);
  ...
  lower[(byte)'ฅ']='ค'; /* PORT x64: indice >=0x80 con char signed indexaba fuera del array */
...
byte * next_lexico(byte * _source, int coment, int linea) { // No genera nunca errores
  ...
  switch ((int)lex_case[*_source]) {
    ...
    case l_id :
      ...
      ivnom.b++; _source--;
      ptr=&vhash[h];
      /* PORT x64: el nombre del nodo vive tras DOS punteros (next+token),
       * luego es _ivnom+2*sizeof(byte*) (+16), no +8. */
      while (*ptr && strcmp((char *)(ptr+2),(char *)_ivnom+2*sizeof(byte*))) ptr=(unsigned char **)*ptr;
      if (!strcmp((char *)(ptr+2),(char *)_ivnom+2*sizeof(byte*))) { // id encontrado
...
void lexico(void) {
  ...
  switch ((int)lex_case[*_source]) { // Puntero a un lex_ele o l_???
    ...
    case l_id :
      ...
      ptr=&vhash[h];
      /* PORT x64: nombre del nodo a +2*sizeof(byte*) (ver next_lexico). */
      while (*ptr && strcmp((char *)(ptr+2),(char *)_ivnom+2*sizeof(byte*))) ptr=(byte**)*ptr;
      if (!strcmp((byte *)(ptr+2),_ivnom+2*sizeof(byte*))) { // id encontrado
        ivnom.b=_ivnom; // lo saca de vnom
        pieza=(int)*(ptr+1);
        ...
        /* PORT x64: slot token/objeto a +sizeof(byte*) (era +4) y nombre
         * a +2*sizeof(byte*) (era +8); los offsets viejos dejaban el
         * puntero al objeto desalineado y corrompian el slot next. */
        *ptr=_ivnom; ptr_o=(void*)(_ivnom+sizeof(byte*)); *ptr_o=o=iobj++; pieza=p_id; // id nuevo
        (*o).name=(byte*)_ivnom+2*sizeof(byte*);
        (*o).member=member;
        if (parametros) (*o).bloque=bloque_actual;
        if (num_obj++==max_obj) c_error(0,102);
      } break;
...
//          test_buffer(&mem,&imem_max,imem);
      ...
      if (*(imemptr-(int)mem+(int)frm)==1 || tipo==1) { // Un string en words (el puntero short)
        ...
        imemptr=(byte*)(((size_t)imemptr+1)&~(size_t)1); /* PORT x64: (int) truncaba el puntero */ // Lo hace par
        *((word*)imemptr)=valor;
        imemptr+=2;
        imem=((int)imemptr-(int)mem+3)/4;
        if (*(imemptr-(int)mem+(int)frm)!=1 && tipo!=1) imemptr=(byte*)&mem[imem];
...
//      test_buffer(&mem,&imem_max,(((int)oimemptr-(int)mem)+len*dup+3)/4);
      while (--dup) {
        for (n=0;n<len;n++) {
          ...
          if (*(byte*)((byte*)frm+(imemptr-(byte*)mem))==1) { /* PORT x64: la suma en int perdia la mitad alta de frm */
            if (*(byte*)((byte*)frm+(oimemptr+n-(byte*)mem))!=1) c_error(3,136); /* PORT x64: idem */
          } else if (*(byte*)((byte*)frm+(oimemptr+n-(byte*)mem))==1) c_error(3,136); /* PORT x64: idem */

          // Comprueba que los bytes caigan sobre bytes
          if (*(byte*)((byte*)frm+(imemptr-(byte*)mem))==2) { /* PORT x64: idem */
            if (*(byte*)((byte*)frm+(oimemptr+n-(byte*)mem))!=2) c_error(3,136); /* PORT x64: idem */
          } else if (*(byte*)((byte*)frm+(oimemptr+n-(byte*)mem))==2) c_error(3,136); /* PORT x64: idem */
...
void plexico(void) {
  ...
  switch ((int)lex_case[*_source]) { // Puntero a un lex_ele o l_???
    ...
    case l_id :
      ...
      ptr=&vhash[h];
      /* PORT x64: nombre del nodo a +2*sizeof(byte*) (ver next_lexico). */
      while (*ptr && strcmp((byte *)(ptr+2),_ivnom+2*sizeof(byte*))) ptr=(void*)*ptr;
      if (!strcmp((byte *)(ptr+2),_ivnom+2*sizeof(byte*))) { // id encontrado
        ivnom.b=_ivnom; // lo saca de vnom
        pieza=(int)*(ptr+1);
        if (pieza<256 && pieza>=0) { // palabra reservada (token)
