		 ...
		 struct PicInfo *pic, struct Region *pRegion)
{
  ...
  int i,pp;
	...
	if (Top<Bot) {	// CurTop<CurBot
//FER
//  if (prev->Top<prev->Bot&&Top<prev->Bot&&Bot>prev->Top) {// normal update
		if (prev->Top<prev->Bot) {// normal update
			if (prev->Top>Top) {	// init
				hl=&FLines[Top];
				for(i=Top;i<prev->Top;i++,hl++) {
					hl->LeftCol=Col;
					if (hl->pRegion==pRegion) continue;
					scan=INT_FIX(i-CurView->Horizon);
					if (scan==0) scan+=INT_FIX(1);
					if (scan>0) H=pRegion->FloorH;
					else H=pRegion->CeilH;
					hl->pRegion=pRegion;
					hl->RawPtr=pic->Raw;
          ...
          if (VPE_fog)
          {
            pp=FixDiv(t,CurView->FLen)-CurView->FIni;
            if(pp>0) fade+=pp;
          }
					fade>>=16;
					if (fade>=Pal.PH.NumShades)
						...
						if (fade<0) fade=0;
					hl->PalPtr=Pal.Tables[CurView->Table]+(fade<<8);
