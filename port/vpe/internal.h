...
struct FLine {
  ...
  /* PORT: el original ASM (draw_fa.asm/floop.inc) empaquetaba X e Y en un
   * unico DWORD Coord/Delta (X en los 16 bits bajos >>Width2, Y en los 16
   * bits altos <<(16-Width2)) para poder extraer el indice de textura con
   * un truco de registros (shld/shld) en un paso. El reemplazo en C
   * (draw_span.c) no necesita esa compresion -- U/V son las coordenadas de
   * textura (FIXED 16.16) tal cual, dU/dV su paso por columna. */
  FIXED  U, V, dU, dV;
