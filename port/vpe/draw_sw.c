...
// Misc stuff
// PORT: PickVX puede ser -1 (sin pick activo, Engine.PickX=-1 por
// defecto) -- el original leia CurLevel->Clip[-1] incondicionalmente
// (un byte fuera de array, benigno en DOS real-mode) y solo USABA
// PickTop/PickBot mas abajo bajo "if (PickVFlag)". ASan lo detecta como
// heap-buffer-overflow real; se guarda la lectura con la misma condicion
// que ya protege su uso.
	if (PickVFlag) {
