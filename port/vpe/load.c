...
//  Gen.BackAngle=zgen->BackAngle*DEG90/90;
  ...
  // Prepare WallPtrs
  // PORT: era "Walls.Number*2*4" (2 punteros por pared, asumiendo 4 bytes
  // -- el tamano de un puntero en el DOS 32-bit original). En x64 un
  // puntero mide 8 bytes: con la constante original el buffer se quedaba
  // a la mitad (heap-buffer-overflow confirmado con ASan al escribir
  // WallPtrs[NumWallPtrs] mas abajo).
  WallPtrs=(struct Wall **)CacheAlloc(Walls.Number*2*sizeof(struct Wall *));
...
void LoadPalette(char *palname)
{
  ...
  // PORT: era "(DWORD)Pal.MemTables" (trunca el puntero a 32 bits en x64,
  // access-violation confirmado con ASan en set_fog_table al escribir a
  // traves de Pal.Tables[0]) -- misma alineacion a 256 bytes con
  // aritmetica de puntero de 64 bits.
  Pal.Tables[0]=(BYTE *)(((uintptr_t)Pal.MemTables+255)&~(uintptr_t)0xFF);
...
{
  int i,j;
  // PORT: era "char *tabla_color" -- mismo patron de bug ya visto en
  // diveffec.cpp (E10, ver docs): valores de indice de paleta 0-255
  // guardados en un char CON SIGNO (default MSVC). Para valores >=128 el
  // wraparound a negativo, multiplicado por 256 mas abajo, produce un
  // offset negativo enorme sobre "ghost" -- confirmado con ASan
  // (heap-use-after-free por leer muy fuera de los limites de "ghost").
  BYTE *tabla_color;
