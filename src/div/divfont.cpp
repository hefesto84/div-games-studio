...
int an=v.an,al=v.al,x,y,tal=24,tan=41;
        if (big) {tan*=big2;tal*=big2; }
        for(y=0;y<tal;y++)
                for(x=0;x<tan;x++)
                        ptr[(y*an+x)+((86*big2)*an+(64*big2))]=Text1[y*tan+x];
        for(y=0;y<tal;y++)
                for(x=0;x<tan;x++)
                        ptr[(y*an+x)+((86*big2)*an+(124*big2))]=Text2[y*tan+x];
        for(y=0;y<tal;y++)
                for(x=0;x<tan;x++)
                        ptr[(y*an+x)+((86*big2)*an+(4*big2))]=Text3[y*tan+x];
        ...
        if((wmouse_y>86)&&(wmouse_y<86+24)&&(mouse_b))
        {
                if((wmouse_x>64)&&(wmouse_x<64+41))
                {
                        ...
                        if(SelColorOk)
                        {
                                ...
                                ShowText();
                                wmouse_x=-1;
                                if (Text01!=NULL) { free(Text01); Text01=NULL; }
                        }
                        return;
                }
                if((wmouse_x>124)&&(wmouse_x<124+41))
                {
                        ...
                        if(SelColorOk)
                        {
                                ...
                                ShowText();
                                wmouse_x=-1;
                                if (Text02!=NULL) { free(Text02); Text02=NULL; }
                        }
                        return;
                }
                if((wmouse_x>4)&&(wmouse_x<4+41))
                {
                        ...
                        if(SelColorOk)
                        {
                                ...
                                ShowText();
                                wmouse_x=-1;
                                if (Text03!=NULL) { free(Text03); Text03=NULL; }
                        }
                        return;
                }
        }
          ...
          if((wmouse_y>86)&&(wmouse_y<86+24))
          {
                if((wmouse_x>64)&&(wmouse_x<64+41))
                {
                        ...
                        else
                        {
                                ...
                                memcpy(Text01,ventana[1].mapa->map,Text1Anc*Text1Alt);
                                x=0;
                                y=0;
                                for(py=0;py<ventana[1].mapa->map_al;py+=ventana[1].mapa->map_al/(float)tal)
                                {
                                        x=0;
                                        for(px=0;px<ventana[1].mapa->map_an;px+=ventana[1].mapa->map_an/(float)tan)
                                                Text1[y*tan+x++]=ventana[1].mapa->map[ (int)py*ventana[1].mapa->map_an+(int)px];
                                        y++;
                                }
                        }
                        ShowText();
                ...
                if((wmouse_x>124)&&(wmouse_x<124+41))
                {
                        ...
                        else
                        {
                                ...
                                memcpy(Text02,ventana[1].mapa->map,Text2Anc*Text2Alt);
                                x=0;y=0;
                                for(py=0;py<ventana[1].mapa->map_al;py+=ventana[1].mapa->map_al/(float)tal)
                                {
                                        x=0;
                                        for(px=0;px<ventana[1].mapa->map_an;px+=ventana[1].mapa->map_an/(float)tan)
                                                Text2[y*tan+x++]=ventana[1].mapa->map[(int)py*ventana[1].mapa->map_an+(int)px];
                                        y++;
                                }
                        }
                        ShowText();
                ...
                if((wmouse_x>4)&&(wmouse_x<4+41))
                {
                        ...
                        else
                        {
                                ...
                                memcpy(Text03,ventana[1].mapa->map,Text3Anc*Text3Alt);
                                x=0;y=0;
                                for(py=0;py<ventana[1].mapa->map_al;py+=ventana[1].mapa->map_al/(float)tal)
                                {
                                        x=0;
                                        for(px=0;px<ventana[1].mapa->map_an;px+=ventana[1].mapa->map_an/(float)tan)
                                                Text3[y*tan+x++]=ventana[1].mapa->map[(int)py*ventana[1].mapa->map_an+(int)px];
                                        y++;
                                }
                        }
                        ShowText();
...
void Fonts0(void) {
  ...
  FILE *f;
        ...
        v.an=169;
        v.al=179-17;
        v.paint_handler=(int)Fonts1;
        v.click_handler=(int)Fonts2;
        v.close_handler=(int)Fonts3;
        v.titulo=texto[83];
        v.nombre=texto[83];
        lfontsizes.creada=0;
        lfontsizes.maximo=8;
...
byte cColor;
        ...
        if((wmouse_y>10)&&(wmouse_y<138)&&(wmouse_x>2)&&(wmouse_x<130))
        {
                ...
                if(mouse_b)
                {
                        ...
                        {
                          wbox(v.ptr,an,al,SelColorFont,(SelColorFont%16)*8+4,(SelColorFont/16)*8+12,3,3);
                          SelColorFont=cColor;
                          if(dac[SelColorFont*3]+dac[SelColorFont*3+1]+dac[SelColorFont*3+2]<(32+32+32))
                                wbox(v.ptr,an,al,c4,(SelColorFont%16)*8+4,(SelColorFont/16)*8+12,3,3);
                          else
...
void Selcolor0(void)
{
        ...
        v.al=157;
        v.titulo=texto[78];
        v.paint_handler=(int)Selcolor1;
        v.click_handler=(int)Selcolor2;
        v.close_handler=(int)Selcolor3;
...
*/
        ...
        if(v.al>200)
                v.al=200;
        v.paint_handler=(int)Preview1;
        v.click_handler=(int)Preview2;
        v.titulo=texto[80];
...
void GenFont0(void)
{
        ...
        v.al=103;
        v.titulo=texto[852];
        v.paint_handler=(int)GenFont1;
        v.click_handler=(int)GenFont2;
        v.close_handler=(int)GenFont3;
...
*/
        ...
        if(v.al>200)
                v.al=200;
        v.paint_handler=(int)Preview21;
        v.click_handler=(int)Preview22;
        v.titulo=texto[98];
        _button(100,v.an/2,v.al-14,1);
}
/////////////////////////////////////////////////////////////////////////////
...
void crear_texto_prueba(char * s, char flags) {
  ...
  *s=0;
  ...
  if(flags&NUM) numbits++;
  if(flags&MAY) numbits++;
  if(flags&MIN) numbits++;
  if(flags&SIM) numbits++;
  if(flags&EXT) numbits++;

  switch(numbits) {
    case 0: return;
    case 1: numchar=8; break;
    case 2: numchar=4; break;
    case 3: numchar=3; break;
    case 4: numchar=2; break;
    case 5: numchar=2; break;
  }
...
void ShowFont1(void)
{
  ...
  if (TamaX>ancho_w*big2 || TamaY>alto_w*big2)
  {
    ...
    for(y=0;y<alto;y++)
    {
      ...
      for(x=0;x<ancho;x++)
      {
        temp2[y*ancho+x] = temp[((int)a)*TamaX+(int)b];
        b+=coefredx;
      }
      a+=coefredy;
    }
...
int Length;
        ...
        v.paint_handler=(int)ShowFont1;
        v.click_handler=(int)ShowFont2;
        v.close_handler=(int)ShowFont3;
        ...
        else
        {
                ...
                for(py=0;py<Text1Alt;py+=Text1Alt/(float)tal)
                {
                        ...
                        for(px=0;px<Text1Anc;px+=Text1Anc/(float)tan)
                                Text1[y*tan+x++]=Text01[(int)py*Text1Anc+(int)px];
                        y++;
                }
        }
//---------------------------------------------------------------
        ...
        else
        {
                ...
                for(py=0;py<Text2Alt;py+=Text2Alt/(float)tal)
                {
                        ...
                        for(px=0;px<Text2Anc;px+=Text2Anc/(float)tan)
                                Text2[y*tan+x++]=Text02[(int)py*Text2Anc+(int)px];
                        y++;
                }
        }
//---------------------------------------------------------------
        ...
        else
        {
                ...
                for(py=0;py<Text3Alt;py+=Text3Alt/(float)tal)
                {
                        ...
                        for(px=0;px<Text3Anc;px+=Text3Anc/(float)tan)
                                Text3[y*tan+x++]=Text03[(int)py*Text3Anc+(int)px];
                        y++;
                }
        }
//---------------------------------------------------------------
