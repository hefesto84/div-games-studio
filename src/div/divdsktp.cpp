...
int nueva_ventana_carga(int init_handler,int nx,int ny)
{
  ...
  int n,m,x,y,an,al;
  int vtipo;
  long lon_ptr; /* PORT: bytes realmente reservados para v.ptr */
...
*/
    ...
    if (big) if (v.an>0) { v.an=v.an*big2; v.al=v.al*big2; } else v.an=-v.an;
...
*/
    lon_ptr=(long)an*al;
    if ((ptr=malloc(lon_ptr))!=NULL) { // Ventana, free en cierra_ventana
...
*/
      ...
      memset(ptr,c0,lon_ptr); if (big) { an/=big2; al/=big2; }
...
*/
      if(!VidModeChanged) {
        ...
        /* PORT: la geometria guardada en session.dtf solo se acepta si
         * cabe en el buffer que se acaba de reservar. Una sesion escrita
         * por un build con otra escala de letra (big2) trae an/al mayores
         * que los recalculados ahora, y entonces cada volcado posterior
         * (wvolcado desde actualiza_caja) lee fuera del bloque. */
