...
/* PORT: strchr(nombre,'.') devuelve NULL si el fichero no tiene
 * extension (LICENSE, makefile, Vagrantfile...); strupr(NULL) hace
 * abortar el proceso en la UCRT de MSVC (fastfail 0xC0000409), a
 * diferencia del CRT de Watcom/DOS original. Mismo comportamiento que
 * el original para nombres CON extension; sin extension se compara
 * contra una cadena vacia, que nunca coincide con ninguna extension
 * real (tratado igual que un tipo de fichero desconocido). */
...
void crear_un_thumb_MAP(struct t_listboxbr * l){
  ...
  if (l->maximo) {
    ...
    if (estado==1) { // Se comienza a leer un nuevo thumbnail
      ...
      if (strcmp(mayus_extension(l->lista+(l->lista_an*num)),".MAP") &&
          strcmp(mayus_extension(l->lista+(l->lista_an*num)),".PCX") &&
          strcmp(mayus_extension(l->lista+(l->lista_an*num)),".BMP") &&
          strcmp(mayus_extension(l->lista+(l->lista_an*num)),".JPG") &&
          strcmp(mayus_extension(l->lista+(l->lista_an*num)),".JPE")) {
...
void crear_un_thumb_PAL(struct t_listboxbr * l)
{
  ...
  if (l->maximo && incremento>=16384)
  {
    ...
    if (!strcmp(mayus_extension(l->lista+(l->lista_an*num)),".FPG"))
      tipo=1;
    else if(!strcmp(mayus_extension(l->lista+(l->lista_an*num)),".FNT"))
      tipo=2;
    else if(!strcmp(mayus_extension(l->lista+(l->lista_an*num)),".PCX"))
      tipo=3;
    else if(!strcmp(mayus_extension(l->lista+(l->lista_an*num)),".BMP"))
      tipo=4;
    else if(!strcmp(mayus_extension(l->lista+(l->lista_an*num)),".MAP"))
      tipo=5;
    else if(!strcmp(mayus_extension(l->lista+(l->lista_an*num)),".PAL"))
      tipo=6;
    else if(!strcmp(mayus_extension(l->lista+(l->lista_an*num)),".JPG"))
      tipo=7;
    else if(!strcmp(mayus_extension(l->lista+(l->lista_an*num)),".JPE"))
      tipo=7;
    else
    {
      estado=0;
...
void crear_un_thumb_FNT(struct t_listboxbr * l)
{
  ...
  if (l->maximo)
  {
    ...
    if (estado==1)
    {
      if(!strcmp(mayus_extension(l->lista+(l->lista_an*num)),".FNT"));
      else
      {
        estado=0;
        thumb[num].status=-1;
...
void crear_un_thumb_IFS(struct t_listboxbr * l)
{
  ...
  if (l->maximo)
  {
    ...
    if(strcmp(mayus_extension(l->lista+(l->lista_an*num)),".IFS"))
    {
      thumb[num].status=-1;
      return;
    }
...
//    printf("thumb[num].filesize:%d, mem:%d\n", thumb[num].filesize, mem);
      ...
      else if(!strcmp(mayus_extension(l->lista+(l->lista_an*num)),".PCM") );
      else
      {
        fclose(f);
        estado=0;
...
// Fin de conversion
      ...
      else
      {
        ...
        for(x=0;x<thumb[num].an;x++)
        {
          ...
          if (y0<0) y0=0;
          if (y1>=thumb[num].al) y1=thumb[num].al-1;
          y=y0;
          do
          {
...
void dir_abrirbr(void) {
  ...
  qsort(archivo,larchivosbr.maximo,an_archivo,(int (*)(const void *,const void *))strcmp);
