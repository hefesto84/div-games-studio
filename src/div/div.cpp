...
//  remove("DEBUGDIV.TXT");
  ...
  if(argc>2&&(!strcmp(argv[2],"/safe") || !strcmp(argv[2],"/SAFE"))) {
    ...
    #ifdef PORT_IDE
    if (port_setup_bin_valido() && (f=fopen("system\\setup.bin","rb"))!=NULL) { /* PORT: ver port_ide_setup.c */
#else
...
*/
    ...
    if (n<max_windows && ventana[n].primer_plano==0 && arrastrar==4 && v.tipo>=100 && ventana[n].tipo!=2) {
      ...
      if (big) { wmouse_x/=big2; wmouse_y/=big2; }
    ...
    //ฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤ
      ...
      if (mouse_graf==5) {
        if (mouse_b&1) {
          wput(v.ptr,v.an/big2,v.al/big2,v.an/big2-9,2,-45);
          vuelca_ventana(0);
        }
        if (!(mouse_b&1) && (old_mouse_b&1)) {
          if (v.click_handler==(int)menu_principal2) {
            ...
            wput(v.ptr,v.an/big2,v.al/big2,v.an/big2-9,2,-35);
            do read_mouse(); while (mouse_b&1);
      ...
      if (mouse_graf==2) {
        ...
        if (mouse_b&1) {
          if (big) {
            ...
            volcado_parcial(v.x,v.y,7*big2,7*big2);
          } else {
            wput(copia,vga_an,vga_al,v.x,v.y,-48);
...
void actualiza_caja(int x, int y, int an, int al) {
  ...
  if (ventana[n].primer_plano<2) {
    ...
    if (n==0 && mover_ventana) {
      { int save_big=big; big=0;
        wrectangulo(copia,vga_an,vga_al,c4,v.x+8*big2,v.y,v.an-8*big2,v.al);
        big=save_big; }
    } else {
...
void nueva_ventana(int init_handler) {
  ...
  if (!ventana[max_windows-1].tipo) {
    ...
    if (big) if (v.an>0) { v.an*=big2; v.al*=big2; } else v.an=-v.an;
    ...
    if (ptr!=NULL) { // Ventana, free en cierra_ventana
      ...
      memset(ptr,c0,an*al); if (big) { an/=big2; al/=big2; }
...
void inicializacion(void) {
  ...
  // PORT: con big2>2 se sintetiza desde la letra pequena (7 px) x big2,
  // asi 3840 sale a 21 px; si no, se reproduce grande/pequeno tal cual.
  if (big2>2) f=fopen("system\\pequeno.fon","rb");
  else f=fopen(big?"system\\grande.fon":"system\\pequeno.fon","rb");
  if (f==NULL) error(0);
  fseek(f,0,SEEK_END); n=ftell(f);
  if ((ptr=(byte *)malloc(n))!=NULL) {
    fseek(f,0,SEEK_SET); fread(ptr,1,n,f); fclose(f);
  } else { fclose(f); error(0); }
  // PORT: normaliza la letra (dir de 32 bits) y la escala si hace falta
  if (big2<=2) {
    text_font=text_font_build(ptr,big?14:7,1);
  } else {
    text_font=text_font_build(ptr,7,big2);
  }
  free(ptr);
  if (text_font==NULL) error(0);
  ...
  // PORT: letra del editor proporcional a la resoluciขn
  if (big2>1) {
    int sw=font_an,sh=font_al;
    byte *nueva=font_corner_build(font,sw,sh,big2,&font_an,&font_al);
    free(font); font=nueva;
    if (font==NULL) error(0);
  }
  ...
  if (f==NULL) error(0);
    ...
    if (n>0 && (graf_ptr=(byte *)malloc(n))!=NULL) {
      ...
      // PORT: sintetiza los gr ficos del sistema si el factor lo requiere
      if (big && big2>2) {
        int ns;
        byte *synth=graf_build_scaled(graf_ptr,n,big2,&ns);
        free(graf_ptr); graf_ptr=synth; n=ns; ptr=graf_ptr;
        if (graf_ptr==NULL) error(0);
      }
...
void determina_unidades(void) {
  /* PORT: el original enumeraba unidades con INT 21h AX=440Eh via
   * intdos(); el shim intdos() de este port es un no-op literal que no
   * toca `r` (port_ide_pre.h), asi que el bucle original conducia por
   * memoria de pila sin inicializar y unidades[] quedaba con basura
   * (una unica letra bogus repetida, no las unidades reales). Se
   * sustituye por GetLogicalDrives(), el equivalente real en Win32. */
  #ifdef PORT_IDE
...
void Load_Cfgbin() {
  ...
  #ifdef PORT_IDE
  file=port_setup_bin_valido()?fopen("system\\setup.bin","rb"):NULL; /* PORT: ver port_ide_setup.c */
#else
  ...
  // PORT: Escala de letra proporcional a la resoluciขn (base 1280 px de ancho)
  // big2 = maximo entre la base del modo big (2 en grande, 1 en pequeno) y el
  // factor de resolucion floor(vga_an/1280), asi 1920 no cambia (2), 2560=2,
  // 3840=3 (x3 sintetizado), y en un monitor pequeno se conserva el modo big
  // clasico (2) en lugar de quedarse en la base de 640.
  if (vga_an<1280) big2=big?2:1;
  else {
    int fac,base;
    fac=vga_an/1280; if (fac<1) fac=1; if (fac>8) fac=8;
    base=big?2:1;
    big2=fac>base?fac:base;
  }
  if (big2>1) big=1;
  VS_BIG=big;
