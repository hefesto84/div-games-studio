...
/* PORT: mismo aliasing ThumbSprite==MapaSprite que en CargarSprite() --
 * ver el comentario de alli. No se ha reproducido un crash desde este
 * punto (FinalizaGenerador, al cerrar el dialogo), pero el patron es
 * identico, asi que se aplica el mismo fix por consistencia. */
...
/* PORT: ThumbSprite puede apuntar al MISMO bloque que MapaSprite
 * (ver "else temp2=mapa;" mas abajo, cuando el thumbnail no necesita
 * reescalarse) -- liberarlos como si fueran independientes es un doble
 * free() del original de 1999 que aqui si se detecta (STATUS_HEAP_
 * CORRUPTION, 0xC0000374): ocurria al clicar la vista 3D del generador
 * de sprites, porque la segunda pasada de render vuelve a entrar aqui. */
  if(ThumbSprite!=NULL && ThumbSprite!=MapaSprite)
  {
    free(ThumbSprite);
  }
