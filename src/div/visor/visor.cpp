...
/* PORT: sustituye lfset (Watcom #pragma aux, "rep stosw") por C llano --
 * rellena d PALABRAS de 16 bits (no bytes) en buf con el valor c, igual
 * que "rep stosw" (AX=c, ECX=d, EDI=buf). El memset() que dejaba comentado
 * el propio original de 1999 no es equivalente (memset rellena BYTES). */
