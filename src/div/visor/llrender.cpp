...
//extern "C" {
  /* PORT: direccion_textura ya se declara (como intptr_t) via "t.h" */
...
/* PORT: sustituye 5 rutinas #pragma aux (Watcom) por C llano. inicio_division
 * / fin_division encadenaban una fdiv en el stack x87 para solapar su
 * latencia con otro trabajo (truco de rendimiento de un Pentium de 1999,
 * invisible a nivel de C); aqui basta con calcularla directamente. nucleo1/
 * nucleo2 son en realidad UNA sola rutina partida en dos (pushad en nucleo1,
 * popad al final de nucleo2, sin restaurar registros entre medias) --
 * siempre se llaman seguidas ("nucleo1(); nucleo2();"), asi que el estado
 * que compartian via registros aqui se pasa por variables globales. */
static float division_pendiente;
void inicio_division(float *x) { division_pendiente = 1.0f / (*x); }
void fin_division(void) { inversa = division_pendiente; }
void asignacion_u_v(void)
{
  u_0 = (int)lrintf(inversa * fu);
  v_0 = (int)lrintf(inversa * fv);
}
...
/* Estado compartido entre nucleo1() y nucleo2() -- ver comentario arriba. */
static uint32_t nucleo_eax, nucleo_ebx, nucleo_edx;

void nucleo1(void)
{
  nucleo_edx = 0;
  nucleo_ebx = ((uint32_t)mr >> 3) & 0x7e0u;
  nucleo_eax = (uint16_t)color_b;
  nucleo_eax &= 0x0f800u;
  nucleo_eax += nucleo_ebx;
  nucleo_eax += (uint32_t)alfa;
  nucleo_edx = (nucleo_edx & 0xFFFF0000u) | (uint16_t)tabla_rgba[nucleo_eax];
  nucleo_edx <<= 5;
  nucleo_ebx = ((uint32_t)mg >> 3) & 0x7e0u;
}
...
void nucleo2(void)
{
  ...
  nucleo_eax += nucleo_ebx;
  nucleo_eax += (uint32_t)alfa;
  dx = (uint16_t)(nucleo_edx & 0xFFFFu);
  dx = (uint16_t)(dx + (uint16_t)tabla_rgba[nucleo_eax]);
  nucleo_edx = (nucleo_edx & 0xFFFF0000u) | dx;
  nucleo_edx <<= 6;
  nucleo_ebx = ((uint32_t)mb >> 3) & 0x7e0u;
  nucleo_eax = (uint16_t)color_b;
  nucleo_eax &= 0x1fu;
  nucleo_eax <<= 11;
  nucleo_eax += nucleo_ebx;
  ...
  dx = (uint16_t)(dx + (uint16_t)tabla_rgba[nucleo_eax]);
  nucleo_edx = (nucleo_edx & 0xFFFF0000u) | dx;
  *destino = (short)(uint16_t)(nucleo_edx & 0xFFFFu);
}
